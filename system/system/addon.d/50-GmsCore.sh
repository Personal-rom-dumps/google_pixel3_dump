...
addToLog() {
  mkdir -p "$(dirname /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GmsCore_addon.log)";
  echo "$1" >> /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GmsCore_addon.log
}

addToLog "Execute Log for GmsCore with commands: $@"
...
if [ "$execute_config" = "0" ]; then
  if [ -f "$S/addon.d/50-GmsCore.sh" ]; then
    ui_print "- Deleting up GmsCore.sh"
    rm -rf $S/addon.d/50-GmsCore.sh
    rm -rf $T/addon.d/50-GmsCore.sh
  fi
  exit 1
fi
...
case "$1" in
 pre-backup)
   rm -rf "/sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GmsCore_addon.log"
 ;;
 backup)
   ui_print "- Backing up GmsCore"
 ...
 ;;
 restore)
   ui_print "- Restoring GmsCore"
   list_files | while read FILE REPLACEMENT; do
     R=""
     [ -n "$REPLACEMENT" ] && R="$S/$REPLACEMENT"
     [ -f "$C/$S/$FILE" ] && restore_file $S/"$FILE" "$R"
   done
   addToLog "Deleting Aosp apps in GmsCore"
   addToLog "Removing Files from Rom Source in GmsCore"
   clean_recursive "PrebuiltGmsCoreQt"
   clean_recursive "PrebuiltGmsCoreRvc"
   clean_recursive "GmsCore"
   addToLog "Running Debloater in GmsCore"
