...
addToLog() {
  mkdir -p "$(dirname /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_ExtraFiles_addon.log)";
  echo "$1" >> /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_ExtraFiles_addon.log
}

addToLog "Execute Log for ExtraFiles with commands: $@"
...
etc/default-permissions/nikgapps-permissions.xml
...
etc/sysconfig/pixel_experience_2020.xml
etc/sysconfig/google-staged-installer-whitelist.xml
etc/sysconfig/dialer_experience.xml
etc/sysconfig/wellbeing.xml
etc/sysconfig/preinstalled-packages-platform-handheld-product.xml
etc/sysconfig/pixel.xml
etc/sysconfig/pixel_experience_2020_midyear.xml
etc/sysconfig/nexus.xml
etc/sysconfig/nga.xml
etc/sysconfig/backup.xml
...
etc/sysconfig/preinstalled-packages-platform-overlays.xml
etc/sysconfig/pixel_2019_exclusive.xml
etc/sysconfig/google-hiddenapi-package-whitelist.xml
etc/sysconfig/pixel_experience_2017.xml
etc/sysconfig/google_build.xml
lib64/libgdx.so
etc/permissions/privapp-permissions-google-product.xml
etc/permissions/privapp-permissions-google-comms-suite.xml
etc/permissions/NikGapps-privapp-permissions-google.xml
etc/permissions/com.google.android.dialer.support.xml
etc/permissions/privapp-permissions-google-system-ext.xml
etc/permissions/split-permissions-google.xml
etc/permissions/privapp-permissions-google-p.xml
etc/permissions/privapp-permissions-hotword.xml
overlay/GmsConfigOverlayCommon.apk
overlay/GmsContactsProviderOverlay.apk
overlay/GmsConfigOverlayGSA.apk
overlay/GmsConfigOverlayComms.apk
...
if [ "$execute_config" = "0" ]; then
  if [ -f "$S/addon.d/50-ExtraFiles.sh" ]; then
    ui_print "- Deleting up ExtraFiles.sh"
    rm -rf $S/addon.d/50-ExtraFiles.sh
    rm -rf $T/addon.d/50-ExtraFiles.sh
  fi
  exit 1
fi
...
case "$1" in
 pre-backup)
   rm -rf "/sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_ExtraFiles_addon.log"
 ;;
 backup)
   ui_print "- Backing up ExtraFiles"
 ...
 ;;
 restore)
   ui_print "- Restoring ExtraFiles"
   list_files | while read FILE REPLACEMENT; do
     R=""
     [ -n "$REPLACEMENT" ] && R="$S/$REPLACEMENT"
     [ -f "$C/$S/$FILE" ] && restore_file $S/"$FILE" "$R"
   done
   addToLog "Deleting Aosp apps in ExtraFiles"
   addToLog "Removing Files from Rom Source in ExtraFiles"
   addToLog "Running Debloater in ExtraFiles"
