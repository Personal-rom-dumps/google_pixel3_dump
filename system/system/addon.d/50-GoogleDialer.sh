...
addToLog() {
  mkdir -p "$(dirname /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GoogleDialer_addon.log)";
  echo "$1" >> /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GoogleDialer_addon.log
}

addToLog "Execute Log for GoogleDialer with commands: $@"
...
if [ "$execute_config" = "0" ]; then
  if [ -f "$S/addon.d/50-GoogleDialer.sh" ]; then
    ui_print "- Deleting up GoogleDialer.sh"
    rm -rf $S/addon.d/50-GoogleDialer.sh
    rm -rf $T/addon.d/50-GoogleDialer.sh
  fi
  exit 1
fi
...
case "$1" in
 pre-backup)
   rm -rf "/sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GoogleDialer_addon.log"
 ;;
 backup)
   ui_print "- Backing up GoogleDialer"
 ...
 ;;
 restore)
   ui_print "- Restoring GoogleDialer"
   list_files | while read FILE REPLACEMENT; do
     R=""
     [ -n "$REPLACEMENT" ] && R="$S/$REPLACEMENT"
     [ -f "$C/$S/$FILE" ] && restore_file $S/"$FILE" "$R"
   done
   addToLog "Deleting Aosp apps in GoogleDialer"
   clean_recursive "Dialer"
   addToLog "Removing Files from Rom Source in GoogleDialer"
   addToLog "Running Debloater in GoogleDialer"
