...
addToLog() {
  mkdir -p "$(dirname /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GoogleContactsSyncAdapter_addon.log)";
  echo "$1" >> /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GoogleContactsSyncAdapter_addon.log
}

addToLog "Execute Log for GoogleContactsSyncAdapter with commands: $@"
...
if [ "$execute_config" = "0" ]; then
  if [ -f "$S/addon.d/50-GoogleContactsSyncAdapter.sh" ]; then
    ui_print "- Deleting up GoogleContactsSyncAdapter.sh"
    rm -rf $S/addon.d/50-GoogleContactsSyncAdapter.sh
    rm -rf $T/addon.d/50-GoogleContactsSyncAdapter.sh
  fi
  exit 1
fi
...
case "$1" in
 pre-backup)
   rm -rf "/sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GoogleContactsSyncAdapter_addon.log"
 ;;
 backup)
   ui_print "- Backing up GoogleContactsSyncAdapter"
 ...
 ;;
 restore)
   ui_print "- Restoring GoogleContactsSyncAdapter"
   list_files | while read FILE REPLACEMENT; do
     R=""
     [ -n "$REPLACEMENT" ] && R="$S/$REPLACEMENT"
     [ -f "$C/$S/$FILE" ] && restore_file $S/"$FILE" "$R"
   done
   addToLog "Deleting Aosp apps in GoogleContactsSyncAdapter"
   addToLog "Removing Files from Rom Source in GoogleContactsSyncAdapter"
   addToLog "Running Debloater in GoogleContactsSyncAdapter"
