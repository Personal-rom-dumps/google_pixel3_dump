...
addToLog() {
  mkdir -p "$(dirname /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GoogleClock_addon.log)";
  echo "$1" >> /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GoogleClock_addon.log
}

addToLog "Execute Log for GoogleClock with commands: $@"
...
if [ "$execute_config" = "0" ]; then
  if [ -f "$S/addon.d/50-GoogleClock.sh" ]; then
    ui_print "- Deleting up GoogleClock.sh"
    rm -rf $S/addon.d/50-GoogleClock.sh
    rm -rf $T/addon.d/50-GoogleClock.sh
  fi
  exit 1
fi
...
case "$1" in
 pre-backup)
   rm -rf "/sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GoogleClock_addon.log"
 ;;
 backup)
   ui_print "- Backing up GoogleClock"
 ...
 ;;
 restore)
   ui_print "- Restoring GoogleClock"
   list_files | while read FILE REPLACEMENT; do
     R=""
     [ -n "$REPLACEMENT" ] && R="$S/$REPLACEMENT"
     [ -f "$C/$S/$FILE" ] && restore_file $S/"$FILE" "$R"
   done
   addToLog "Deleting Aosp apps in GoogleClock"
   clean_recursive "DeskClock"
   addToLog "Removing Files from Rom Source in GoogleClock"
   addToLog "Running Debloater in GoogleClock"
