...
addToLog() {
  mkdir -p "$(dirname /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GoogleCalendarSyncAdapter_addon.log)";
  echo "$1" >> /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GoogleCalendarSyncAdapter_addon.log
}

addToLog "Execute Log for GoogleCalendarSyncAdapter with commands: $@"
...
if [ "$execute_config" = "0" ]; then
  if [ -f "$S/addon.d/50-GoogleCalendarSyncAdapter.sh" ]; then
    ui_print "- Deleting up GoogleCalendarSyncAdapter.sh"
    rm -rf $S/addon.d/50-GoogleCalendarSyncAdapter.sh
    rm -rf $T/addon.d/50-GoogleCalendarSyncAdapter.sh
  fi
  exit 1
fi
...
case "$1" in
 pre-backup)
   rm -rf "/sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GoogleCalendarSyncAdapter_addon.log"
 ;;
 backup)
   ui_print "- Backing up GoogleCalendarSyncAdapter"
 ...
 ;;
 restore)
   ui_print "- Restoring GoogleCalendarSyncAdapter"
   list_files | while read FILE REPLACEMENT; do
     R=""
     [ -n "$REPLACEMENT" ] && R="$S/$REPLACEMENT"
     [ -f "$C/$S/$FILE" ] && restore_file $S/"$FILE" "$R"
   done
   addToLog "Deleting Aosp apps in GoogleCalendarSyncAdapter"
   addToLog "Removing Files from Rom Source in GoogleCalendarSyncAdapter"
   addToLog "Running Debloater in GoogleCalendarSyncAdapter"
