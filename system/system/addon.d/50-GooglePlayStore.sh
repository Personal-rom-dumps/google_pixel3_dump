...
addToLog() {
  mkdir -p "$(dirname /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GooglePlayStore_addon.log)";
  echo "$1" >> /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GooglePlayStore_addon.log
}

addToLog "Execute Log for GooglePlayStore with commands: $@"
...
list_files() {
cat <<EOF
priv-app/Phonesky/split_phonesky_data_loader.apk
priv-app/Phonesky/Phonesky.apk
priv-app/Phonesky/split_config.en.apk
priv-app/Phonesky/split_phonesky_data_loader.config.arm64_v8a.apk
priv-app/Phonesky/split_config.arm64_v8a.apk
...
if [ "$execute_config" = "0" ]; then
  if [ -f "$S/addon.d/50-GooglePlayStore.sh" ]; then
    ui_print "- Deleting up GooglePlayStore.sh"
    rm -rf $S/addon.d/50-GooglePlayStore.sh
    rm -rf $T/addon.d/50-GooglePlayStore.sh
  fi
  exit 1
fi
...
case "$1" in
 pre-backup)
   rm -rf "/sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GooglePlayStore_addon.log"
 ;;
 backup)
   ui_print "- Backing up GooglePlayStore"
 ...
 ;;
 restore)
   ui_print "- Restoring GooglePlayStore"
   list_files | while read FILE REPLACEMENT; do
     R=""
     [ -n "$REPLACEMENT" ] && R="$S/$REPLACEMENT"
     [ -f "$C/$S/$FILE" ] && restore_file $S/"$FILE" "$R"
   done
   addToLog "Deleting Aosp apps in GooglePlayStore"
   addToLog "Removing Files from Rom Source in GooglePlayStore"
   addToLog "Running Debloater in GooglePlayStore"
