...
addToLog() {
  mkdir -p "$(dirname /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GoogleServicesFramework_addon.log)";
  echo "$1" >> /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GoogleServicesFramework_addon.log
}

addToLog "Execute Log for GoogleServicesFramework with commands: $@"
...
list_files() {
cat <<EOF
etc/permissions/com.google.android.gsf.xml
...
if [ "$execute_config" = "0" ]; then
  if [ -f "$S/addon.d/50-GoogleServicesFramework.sh" ]; then
    ui_print "- Deleting up GoogleServicesFramework.sh"
    rm -rf $S/addon.d/50-GoogleServicesFramework.sh
    rm -rf $T/addon.d/50-GoogleServicesFramework.sh
  fi
  exit 1
fi
...
case "$1" in
 pre-backup)
   rm -rf "/sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GoogleServicesFramework_addon.log"
 ;;
 backup)
   ui_print "- Backing up GoogleServicesFramework"
 ...
 ;;
 restore)
   ui_print "- Restoring GoogleServicesFramework"
   list_files | while read FILE REPLACEMENT; do
     R=""
     [ -n "$REPLACEMENT" ] && R="$S/$REPLACEMENT"
     [ -f "$C/$S/$FILE" ] && restore_file $S/"$FILE" "$R"
   done
   addToLog "Deleting Aosp apps in GoogleServicesFramework"
   addToLog "Removing Files from Rom Source in GoogleServicesFramework"
   addToLog "Running Debloater in GoogleServicesFramework"
