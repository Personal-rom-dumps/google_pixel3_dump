...
addToLog() {
  mkdir -p "$(dirname /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_DigitalWellbeing_addon.log)";
  echo "$1" >> /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_DigitalWellbeing_addon.log
}

addToLog "Execute Log for DigitalWellbeing with commands: $@"
...
list_files() {
cat <<EOF
etc/permissions/com.google.android.apps.wellbeing.xml
...
if [ "$execute_config" = "0" ]; then
  if [ -f "$S/addon.d/50-DigitalWellbeing.sh" ]; then
    ui_print "- Deleting up DigitalWellbeing.sh"
    rm -rf $S/addon.d/50-DigitalWellbeing.sh
    rm -rf $T/addon.d/50-DigitalWellbeing.sh
  fi
  exit 1
fi
...
case "$1" in
 pre-backup)
   rm -rf "/sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_DigitalWellbeing_addon.log"
 ;;
 backup)
   ui_print "- Backing up DigitalWellbeing"
 ...
 ;;
 restore)
   ui_print "- Restoring DigitalWellbeing"
   list_files | while read FILE REPLACEMENT; do
     R=""
     [ -n "$REPLACEMENT" ] && R="$S/$REPLACEMENT"
     [ -f "$C/$S/$FILE" ] && restore_file $S/"$FILE" "$R"
   done
   addToLog "Deleting Aosp apps in DigitalWellbeing"
   addToLog "Removing Files from Rom Source in DigitalWellbeing"
   addToLog "Running Debloater in DigitalWellbeing"
