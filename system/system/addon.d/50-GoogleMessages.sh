...
addToLog() {
  mkdir -p "$(dirname /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GoogleMessages_addon.log)";
  echo "$1" >> /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GoogleMessages_addon.log
}

addToLog "Execute Log for GoogleMessages with commands: $@"
...
app/PrebuiltBugle/split_config.en.apk
app/PrebuiltBugle/split_config.xxhdpi.apk
app/PrebuiltBugle/split_config.arm64_v8a.apk
EOF
}

 
if [ "$execute_config" = "0" ]; then
  if [ -f "$S/addon.d/50-GoogleMessages.sh" ]; then
    ui_print "- Deleting up GoogleMessages.sh"
    rm -rf $S/addon.d/50-GoogleMessages.sh
    rm -rf $T/addon.d/50-GoogleMessages.sh
  fi
  exit 1
fi
...
case "$1" in
 pre-backup)
   rm -rf "/sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_GoogleMessages_addon.log"
 ;;
 backup)
   ui_print "- Backing up GoogleMessages"
 ...
 ;;
 restore)
   ui_print "- Restoring GoogleMessages"
   list_files | while read FILE REPLACEMENT; do
     R=""
     [ -n "$REPLACEMENT" ] && R="$S/$REPLACEMENT"
     [ -f "$C/$S/$FILE" ] && restore_file $S/"$FILE" "$R"
   done
   addToLog "Deleting Aosp apps in GoogleMessages"
   clean_recursive "RevengeMessages"
   clean_recursive "messaging"
   clean_recursive "Messaging"
   clean_recursive "QKSMS"
   clean_recursive "Mms"
   addToLog "Removing Files from Rom Source in GoogleMessages"
   addToLog "Running Debloater in GoogleMessages"
