...
addToLog() {
  mkdir -p "$(dirname /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_CarrierServices_addon.log)";
  echo "$1" >> /sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_CarrierServices_addon.log
}

addToLog "Execute Log for CarrierServices with commands: $@"
...
etc/permissions/com.google.android.ims.xml
EOF
}

 
if [ "$execute_config" = "0" ]; then
  if [ -f "$S/addon.d/50-CarrierServices.sh" ]; then
    ui_print "- Deleting up CarrierServices.sh"
    rm -rf $S/addon.d/50-CarrierServices.sh
    rm -rf $T/addon.d/50-CarrierServices.sh
  fi
  exit 1
fi
...
case "$1" in
 pre-backup)
   rm -rf "/sdcard/NikGapps/addonLogs/addonlogfiles/NikGapps_CarrierServices_addon.log"
 ;;
 backup)
   ui_print "- Backing up CarrierServices"
 ...
 ;;
 restore)
   ui_print "- Restoring CarrierServices"
   list_files | while read FILE REPLACEMENT; do
     R=""
     [ -n "$REPLACEMENT" ] && R="$S/$REPLACEMENT"
     [ -f "$C/$S/$FILE" ] && restore_file $S/"$FILE" "$R"
   done
   addToLog "Deleting Aosp apps in CarrierServices"
   addToLog "Removing Files from Rom Source in CarrierServices"
   addToLog "Running Debloater in CarrierServices"
